...
fn map_bayc_events(blk: &eth::Block, events: &mut contract::Events) {
        ...
        &mut blk
            ...
            .flat_map(|view| {
                view.receipt
                    ...
                    .filter_map(|log| {
                        if let Some(event) =
                            abi::bayc_contract::events::Approval::match_and_decode(log)
                        {
    ...
    events.bayc_approval_for_alls.append(
        &mut blk
            .receipts()
            .flat_map(|view| {
                view.receipt
                    ...
                    .filter_map(|log| {
                        if let Some(event) =
                            abi::bayc_contract::events::ApprovalForAll::match_and_decode(log)
                        {
    ...
    events.bayc_ownership_transferreds.append(
        &mut blk
            .receipts()
            .flat_map(|view| {
                view.receipt
                    ...
                    .filter_map(|log| {
                        if let Some(event) =
                            abi::bayc_contract::events::OwnershipTransferred::match_and_decode(log)
                        {
        ...
        &mut blk
            ...
            .flat_map(|view| {
                view.receipt
                    ...
                    .filter_map(|log| {
                        if let Some(event) =
                            abi::bayc_contract::events::Transfer::match_and_decode(log)
                        {
