...
> If you choose to use `substreams init` to start you're own project in the future, you'll be prompted to enter an address where the ABI will be fetched for you from Etherscan.
...
- [ ] TODO 2: Pass the address that emmitted the event 🎇 into `token_meta` so token_meta can make the calls to the correct address.

- [ ] TODO 3: Add a check that the name field on `token_meta` contains "Ape".

- [ ] TODO 4: Populate `address` field on the protobuf with the event address.
